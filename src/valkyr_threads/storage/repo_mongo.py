...
def _to_doc(t: Thread) -> dict:
    return {
        ...
        "deps": t.deps,
        "blockers": t.blockers,
        "created_at": t.created_at.replace(microsecond=0).isoformat(),
        "archived": t.archived,
...
class MongoThreadRepo:
    ...
    def upsert(self, t: Thread) -> None:
        self.c_threads.find_one_and_replace({"_id": t.id}, _to_doc(t), upsert=True, return_document=ReturnDocument.AFTER)

    def archive(self, thread_id: str, archived: bool = True) -> None:
        self.c_threads.update_one({"_id": thread_id}, {"$set": {"archived": archived, "updated_at": datetime.now().isoformat()}})

    def save_workspace(self, ws: Workspace) -> None:
        self.c_ws.update_one({"_id": "default"}, {"$set": {"wip_limit": ws.wip_limit, "default_quantum": ws.default_quantum, "updated_at": datetime.now().isoformat()}}, upsert=True)

    def load_workspace(self) -> Workspace:
        d = self.c_ws.find_one({"_id": "default"}) or {}
        return Workspace(wip_limit=int(d.get("wip_limit", 3)), default_quantum=d.get("default_quantum", "50m"))
